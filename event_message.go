...
// FastReply 快速回复
func (m *Message) FastReply(msg string, autoEscape bool) error {
	return m.FastOperation(msg, autoEscape, false, false, false, false, 0)
}

// FastReplyWithAt 快速回复并且@发送者
func (m *Message) FastReplyWithAt(msg string, autoEscape bool) error {
	if m.GroupId <= 0 {
...
// FastDelete 撤回消息
func (m *Message) FastDelete() error {
	if m.GroupId <= 0 {
...
func (m *Message) FastBan(banDuration int) error {
	if m.GroupId <= 0 {
		logger.Warnln("FastReplyWithAt 只能在群组中使用")
		return nil
	}
...
func (m *Message) FastOperation(msg string, autoEscape, atSender, delete, kick, ban bool, banDuration int) error {
	var rep any
	if m.GroupId == 0 {
